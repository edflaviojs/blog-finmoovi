...
async function main() {
  ...
  if (canibal.covered) {
    ...
    warnSkip(topic.topic, `conflita com ${canibal.conflictSlug}`);
    return;
  }
...
`;
  ...
  try {
    ...
    if (skipSeTituloCanibaliza(slugPt, 'sazonal', POSTS_DIR, title)) return;
    ...
    savePost(slugPt, { title, meta, headline, keywords: allKeywords, content: processedContent, imagePath, locale: 'pt', today, translationKey: slugPt });
